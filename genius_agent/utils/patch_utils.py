...
def validate_patch(target: Any, attribute: str) -> bool:
    ...
    if not hasattr(target, attribute):
        logger.warning(
            f"Patch validation failed: '{target}' has no attribute '{attribute}'"
        )
        return False
    return True
...
def apply_patch(
    target: Any, attribute: str, new_value: Any, validate: bool = True
) -> bool:
    """
    ...
    if validate and not validate_patch(target, attribute):
        logger.warning(
            f"Skipping patch for {target.__name__ if hasattr(target, '__name__') else target}.{attribute}"
        )
        return False
    ...
    try:
        ...
        logger.info(
            f"Successfully patched {target.__name__ if hasattr(target, '__name__') else target}.{attribute}"
        )
        return True
