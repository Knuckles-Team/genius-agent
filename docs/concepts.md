...
|------------|------|-------------|
| `CONCEPT:GA-OS.governance.genius` | Core API Client | Primary API client for GeniusAgent Search Engine for Agentic AI! |
| `CONCEPT:GA-OS.governance.genius-2` | MCP Server | Model Context Protocol server entry point |
| `CONCEPT:GA-OS.governance.genius-3` | A2A Agent | Agent-to-Agent protocol server |
...
This project integrates with `agent-utilities` via `CONCEPT:AU-ECO.messaging.native-backend-abstraction` (Unified Toolkit Ingestion). The `genius_agent` MCP server registers its tools with the agent-utilities FastMCP middleware, enabling automatic discovery, telemetry, and Knowledge Graph ingestion of all GENIUS-* concepts.
